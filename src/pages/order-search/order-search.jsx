...
class Search extends Taro.Component {
  ...
  render() {
    const {
      showList, type, typelist, taglist, value,
      buttonTop,
      buttonHeight,
    } = this.state;
    // const i = typelist.findIndex(r => r.type === type)
    const getI = () => {
      return typelist.findIndex(r => r.type === type)
    }
    return (
      <View style={{ paddingTop: `${buttonTop + buttonHeight}px` }}>
        ...
        <View className={styles.nav_out}>
          <View className={styles.top_inner} style={{ marginTop: `${buttonTop}px`, height: `${buttonHeight}px` }}>
            <View className={`iconfont ${styles.go_back_icon}`} onClick={this.goOrderIndex}>&#xe682;</View>
            <View className={styles.font}>订单搜索</View>
          </View>
        </View>
